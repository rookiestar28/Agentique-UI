...
- `@agentique.io/readback` for read-only public resource readback, download metadata normalization, trust state, parser-variant state, agent-native state, and badge projection.
...
- safe download metadata review and user-selected artifact acquisition,
...
- Download acquisition must require a safe destination, bounded redirects, expected size checks, digest checks, atomic writes, and cleanup evidence.
